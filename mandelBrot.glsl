...
vec3 palette(float i) {
    ...
    if (i < 0.25) {
        r = i/0.25;
        return vec3(0,0,0) * (1-r) + vec3(0,0,0.5) * r;
    } else if (i < 0.5) {
        r = (i-0.25)/0.25;
        return vec3(0,0,0.5) * (1-r) + vec3(1,1,1) * r;
    } else if (i < 0.75) {
        r = (i-0.5)/0.25;
        return vec3(1,1,1) * (1-r) + vec3(0.5,0,0) * r;
    }
    r = (i-0.75)/0.25;
    return vec3(0.5,0,0) * (1-r) + vec3(0,0,0) * r;
}

float mandelBrot(double cr, double ci) {
    double zr = cr;
    double zi = ci;
...
void main() {
    float n = mandelBrot(centerx + position.x*whx, centery + position.y*why);
    if (n < maxIters) {
